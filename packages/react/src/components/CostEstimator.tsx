...
export function CostEstimator({
  ...
  const handlePromptTokensChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value) || 0;
    setPromptTokens(value);
  }, []);

  const handleCompletionTokensChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value) || 0;
    ...
    <div style={{ ...styles.container, ...style }} className={className}>
      ...
      {cost && (
        <div style={styles.costBreakdown}>
          <div style={styles.costRow}>
            ...
            <span style={styles.costValue}>{formatCostValue(cost.promptCost)}</span>
          </div>
          <div style={styles.costRow}>
            <span style={styles.costLabel}>Completion Cost</span>
            <span style={styles.costValue}>{formatCostValue(cost.completionCost)}</span>
          </div>
          <div style={{ ...styles.costRow, ...styles.costRowLast }}>
            <span style={styles.costLabel}>Total Cost</span>
            <span style={isFree ? { ...styles.costValue, ...styles.costValueFree } : styles.costValue}>
      ...
      <div style={styles.pricingInfo}>
        ...
        <div style={styles.pricingRow}>
          ...
          <span>${parseFloat(model.pricing.completion).toFixed(6)}/1K tokens</span>
        </div>
      </div>
