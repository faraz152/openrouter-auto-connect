...
import { ParameterDefinition, OpenRouterModel } from './types';
 ...
 */
export function getModelParameters(model: OpenRouterModel): ParameterDefinition[] {
  ...
  for (const paramName of supported) {
    ...
    if (definition) {
      ...
      if (paramName === 'max_tokens' && model.top_provider?.max_completion_tokens) {
        definitions.push({
          ...definition,
...
export function validateParameter(
  ...
  if (value !== undefined && value !== null) {
    ...
    if (definition.min !== undefined && value < definition.min) {
      return { valid: false, error: `${name} must be at least ${definition.min}` };
    }
    if (definition.max !== undefined && value > definition.max) {
      return { valid: false, error: `${name} must be at most ${definition.max}` };
    }

    // Check enum
    if (definition.enum && !definition.enum.includes(value)) {
      return { valid: false, error: `${name} must be one of: ${definition.enum.join(', ')}` };
    }
  }
 ...
 */
export function getDefaultParameters(model: OpenRouterModel): Record<string, any> {
 ...
 */
export function isParameterSupported(model: OpenRouterModel, paramName: string): boolean {
