...
describe('parseOpenRouterError', () => {
  ...
  it('maps network errors', () => {
    const err = parseOpenRouterError({ code: 'ECONNREFUSED', message: 'Connection refused' });
...
describe('isRetryableError', () => {
  it('returns true for retryable error', () => {
    expect(isRetryableError({ code: 'RATE_LIMITED', message: '', retryable: true })).toBe(true);
  });

  it('returns false for non-retryable error', () => {
    expect(isRetryableError({ code: 'INVALID_API_KEY', message: '', retryable: false })).toBe(false);
  });
});
